  ...
  return (
    <>
      ...
      {projectsData.map((project, index) => (
        ...
        >
          <Image
            className="w-full h-auto rounded-xl mb-5 lg:w-[40%] lg:mb-0"
            width={910}
            height={512}
          ...
          <div className="px-4 lg:px-8 lg:py-5">
            ...
            <h2 className="font-bold text-md lg:text-2xl mb-2 lg:mb-3 leading-[1.6]">
              {project.name}
            </h2>
            <p className="mb-4 text-sm xl:text-base">{project.description}</p>
            <ul className="flex flex-wrap text-[12px]">
              {project.tools.map((tool, index) => (
                <li
                  key={index}
                  className="mr-[0.7em] mb-[0.7em] px-[0.9em] py-[0.2em] rounded-[0.3em] bg-accentBg"
                >
                  {tool}
                </li>
              ))}
