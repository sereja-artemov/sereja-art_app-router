  ...
  return (
    <>
        ...
        <div className='container flex gap-x-1.5 items-stretch mx-auto'>
          <nav className='mx-auto w-full left-0 flex gap-x-1.5'>
            <button type='button' className='flex grow lg:grow-0 shrink-0 p-0.5 lg:p-1 border rounded-full'>
              <div className="w-full h-full gap-3 px-4 pr-1 p-0.5 rounded-full bg-slate-50 flex justify-between items-center">
                <Link className='text-black lg:mb-[0.1em]' href='/'>Главная страница</Link>
            ...
            <ul className='hidden h-auto gap-1.5 lg:flex'>
                  { navigationRoutes.map((route, index) => (
                      <li key={index} className="flex items-center px-6 my-0.5 border rounded-full border-gray-200/25 bg-[#2A2A2A] hover:bg-slate-50 hover:text-black">
            ...
            <div className='flex'>
              ...
              {/* Мобильное меню */}
              { isMenuOpen && 
                <ul className='fixed top-20 left-1/2 -translate-x-1/2 w-[90vw] h-auto p-4 py-8 overflow-auto bg-black border lg:hidden rounded-3xl'>
          ...
          <div className='flex items-center justify-between gap-2 border rounded-full px-4 p-0.5'>
            <div>1</div>
            <div>2</div>
          </div>
          <div>
